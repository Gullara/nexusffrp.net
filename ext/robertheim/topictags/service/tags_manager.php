...
class tags_manager
{
	...
	/**
	 * Assigns exactly the given valid tags to the topic (all other tags are removed from the topic and if a tag does
	...
	public function assign_tags_to_topic($topic_id, $valid_tags)
	{
		...
		// garbage collection
		$garbade = (bool) $this->config[prefixes::CONFIG . '_garbage_collection'];
		if (true === $garbade)
		{
			$this->delete_unused_tags();
		}
	 ...
	 * @return array an array of the form array(array('id'=>.. , 'tag'=> ..), array('id'=>.. , 'tag'=> ..), ...) or
	 *                  array(1,2,3,..) if $only_ids==true
	 */
	 ...
	 * and only those which the user is allowed to read.
	 *
	...
	/**
	 * Counts the topics which are tagged with any or all of the given $tags from all forums, where tagging is enabled
	 * and only those which the user is allowed to read.
	 *
	...
	/**
	 * Checks if the given tag matches the configured regex for valid tags, Note that the tag is trimmed to 30
	 * characters before the check! This method also checks if the tag is whitelisted and/or blacklisted if the lists
	 * are enabled.
	 *
	...
	/**
	 * Merges two tags, by assigning all topics of tag_to_delete_id to the tag_to_keep_id and then deletes the
	 * tag_to_delete_id. NOTE: Both tags must exist and this is not checked again!
	 *
